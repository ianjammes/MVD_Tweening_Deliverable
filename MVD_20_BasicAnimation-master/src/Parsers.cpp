...
bool Parsers::parseAnimation(std::string filename) {
	...
	if (file.is_open()){
		...
		if (target_ent != "") {
			...
			//read in keyframes - start from second line automatically
			while (std::getline(file, line)) {
				...
				lm::mat4 translation;
				translation.makeTranslationMatrix(
													stof(w[1]),
													stof(w[2]),
													stof(w[3])
													);
				...
				//rotation matrix
				lm::quat qrot(stof(w[4]), stof(w[5]), stof(w[6]));
				lm::mat4 rotation;
			...
			anim.num_frame = (GLuint)anim.keyframes.size();
			anim.ms_frame = 1000.0f / (float)frames_per_second;
				

		} else {
			std::cout << "ERROR: anim file not reference entity";
			file.close();
			return false;
		}
